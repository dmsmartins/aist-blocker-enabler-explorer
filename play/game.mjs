import {learningCard,journeyStep,discoveryNotice} from './learning.mjs';
...
function prepare(i){
  discoveryQueue.length=0;noticeQueue.length=0;$('delivery').hidden=true;audio.setPlaying(false);index=i;run=createRun(worlds[i],progress);mode='ready';input.clear();renderer.reset(run);$('toast').textContent='';$('pause').disabled=true;$('mapButton').disabled=false;$('orientation').disabled=false;$('restart').disabled=false;gates();hud();
...
function handle(events){let changed=false;for(const ev of events){renderer.react(ev);if(['encounter','collect'].includes(ev.type))noticeQueue.push(discoveryNotice(ev));if(ev.type==='open')showDelivery(ev);
  if(ev.type==='encounter'){changed=true;}
