...
import {readProgress,writeProgress,captureProgress,freshProgress,reconcileProgress} from './storage.mjs';
...
function hud(){
  if(!run)return;const e=currentEncounter(run),selected=run.selected&&data.enablerMap.get(run.selected);
...
function prepare(i){
  index=i;run=createRun(worlds[i],progress);mode='ready';input.clear();renderer.reset(run);$('toast').textContent='';$('pause').disabled=true;$('mapButton').disabled=false;$('orientation').disabled=false;gates();hud();
  const learned=run.inventory.size>0,empty=run.world.epilogue||run.world.unavailable;
  overlay({title:i===0&&!learned?'A little light. A world of possibility.':run.world.alias.toLowerCase().replace(/^./,s=>s.toUpperCase())+'.',text:empty?'Carry what you have learned through the living network. Visit the handover signals, then return to the central portal.':'Explore the islands. Find the barriers. Discover enabling capabilities elsewhere, then return to change the system.',button:learned?'Continue this path →':'Begin the journey →',kicker:`STAGE GATE ${run.world.gate.id} · ${run.world.gate.label}`,note:run.world.epilogue?'No mapped blocker challenges in this gate. This epilogue uses narrative game metaphors.':run.world.unavailable?'Mapped blockers exist, but usable enabling relationships are not available. This is a narrative fallback.':`${run.encounters.length} selected challenges · ${run.world.mappedCount} mapped blockers. Challenges come from the live Explorer.`,map:true});
...
function play(){
  ...
  if(!run)return;mode='playing';input.clear();$('overlay').hidden=true;$('pause').disabled=false;$('pause').textContent='Ⅱ';$('pause').setAttribute('aria-label','Pause game');audio.unlock(progress.settings.sound);lastTime=performance.now();accumulator=0;canvas.focus({preventScroll:true});
  if(innerWidth<600)$('gameShell').scrollIntoView({block:'start',behavior:'instant'});
}
function pause(){if(mode!=='playing')return;mode='paused';input.clear();persist();$('pause').textContent='▶';$('pause').setAttribute('aria-label','Resume game');overlay({title:'A quiet moment.',text:'Your place in the network is saved. Take a look at the map, or continue when you are ready.',button:'Return to the spark →',kicker:run.world.gate.label,note:'No timer. No score. Discover the system at your own pace.',map:true});}
function togglePause(){if(mode==='paused')play();else pause();}
function newPath(){const settings={...progress.settings};progress=freshProgress(String(crypto.getRandomValues(new Uint32Array(1))[0]));progress.settings=settings;progress=reconcileProgress(progress,data);worlds=buildJourney(data,progress.seed);run=null;prepare(0);}

function showPanel(kicker,html){
  if(!$('details').open){dialogPrevious=mode;lastFocus=document.activeElement;if(mode==='playing'){mode='dialog';input.clear();} $('details').showModal();}
  $('dialogKicker').textContent=kicker;$('dialogBody').innerHTML=html;$('details').scrollTop=0;$('closeDialog').focus({preventScroll:true});
}
$('details').addEventListener('close',()=>{if(mode==='dialog'){mode=dialogPrevious;lastTime=performance.now();accumulator=0;}input.clear();if(mode==='playing')canvas.focus({preventScroll:true});else if(lastFocus?.isConnected)lastFocus.focus({preventScroll:true});});
...
$('start').onclick=play;$('pause').onclick=togglePause;$('help').onclick=help;$('mapButton').onclick=showMap;$('orientation').onclick=showMap;$('overlayMap').onclick=showMap;$('knowledge').onclick=()=>showKnowledge();$('inventoryButton').onclick=()=>showInventory();$('touchCycle').onclick=()=>showInventory();
