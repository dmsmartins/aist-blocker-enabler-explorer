...
export function learningCard(event,run){
 ...
 return {kicker:'TOOLS BECOME A PATH',html:`<p class="eyebrow">TOOLS BECOME A PATH</p><h3>${esc(e.blocker.title)}</h3><div class="key-assembly" aria-hidden="true">${unique.map((r,i)=>`<span style="--i:${i}">⚿</span>`).join('')}<b>◇ → ◇</b></div><p><strong>${unique.length} ${unique.length===1?'capability':'capabilities'} connected.</strong> ${esc(excerpt(event.relation?.rationale||unique[0]?.rationale,220))}</p>`};
}
