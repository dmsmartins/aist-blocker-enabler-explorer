import {THEMES,powerStyle} from './config.mjs';
import {platforms,currentEncounter} from './engine.mjs';
...
export class Renderer {
  ...
  react(event){if(event.type==='open'){this.overview=1.8;this.effects.push({x:event.encounter.x,y:event.encounter.y-25,mechanism:event.relation.mechanism,age:0,type:'open'});}if(event.type==='collect')this.effects.push({x:event.pickup.x,y:event.pickup.y-28,mechanism:event.pickup.relation.mechanism,age:0,type:'collect'});if(event.type==='return')this.overview=.8;}
  ...
  encounter(e,run,time){const c=this.ctx,theme=THEMES[run.world.index%THEMES.length],x=e.x+e.dir*205,y=e.y-56,a=e.opened?(this.reduced?1:e.animation):0,noise=1-a,color=e.opened?'#447f79':'#6b8998';
    ...
    if(near)this.label(e.opened?'E · return shortcut':run.inventory.has(run.selected)&&e.relations.some(r=>r.enablerId===run.selected)?'E · change this system':'Another capability is needed',e.x,e.y-72,12,'#315969');
  ...
  render(run,delta,time,mode='playing') {
    ...
    const exit=run.world.exit,ready=run.encounters.length?run.encounters.every(e=>e.opened):run.signals.size===run.world.signals.length;
